/***************************************************************************
 ...
 (no overhead of serialization on the sender).
 ---------------------------------------------------------------------------
...
int main(int argc, char* argv[])
{
	...
	if(rank == 0) // streamer
	{
		...
		for(unsigned int i=0; i< messageSizes.size(); i++)
			std::cout << "size: " << messageSizes[i] << std::endl;

		// send events
		unsigned int idx = 0, counter = 0;
		while(counter++ < numevents)
		{
			MPI::COMM_WORLD.Send(&messageSizes[idx], 1, MPI_UNSIGNED_LONG, 1, 0);
			MPI::COMM_WORLD.Send(messages[idx], messageSizes[idx], MPI_UNSIGNED_CHAR, 1, 0);
